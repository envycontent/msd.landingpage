...
*/
(function(jq) {
	jq.fn.jcaption = function(settings) {
		settings = jq.extend({
			wrapperElement: 'div',
			wrapperClass: 'illustration',
			captionElement: 'p',
			imageAttr: 'alt',
			requireText: true,
			copyStyle: false,
		...
		return jq(this).each(function(){
			...
			jq(this).bind('load', function(){
				...
				if(jq(this).data('loaded')) return false;
				jq(this).data('loaded', true);
				...
				if(image.attr(settings.imageAttr).length > 0 || !settings.requireText){
					...
					var div = jq(this).parent().append('<' + settings.captionElement + '>' + image.attr(settings.imageAttr) + '</' + settings.captionElement + '>');
					
					if(settings.animate){
						jq(this).next().hide();
						jq(this).parent().hover(
						function(){
							jq(this).find('p').animate(settings.show, settings.showDuration);
						},
						function(){
							jq(this).find('p').animate(settings.hide, settings.hideDuration);
						});
					}
